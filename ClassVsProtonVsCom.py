...
def ClassVsProtonVsCom(file_name):

  dataset_file = pd.read_csv(file_name , header = None)
  data_for_graph = pd.read_csv(file_name , sep=',')
  sns.set(font_scale=0.5)
  countplt=sns.catplot(x='NO_OF_CMP', y= 'NO_OF_PROTON', hue = 'chemical_shift_class', data=data_for_graph)
  plt.title('Visualization of Chemical Shift Classes with respect to No of Proton and Compound', fontsize=10)
  plt.xlabel('No. of the Compund', fontsize=10)
...
if __name__ == '__main__':
  if len(sys.argv) > 1:
    ...
    if os.path.exists(folders):
      ...
      ClassVsProtonVsCom(folders)
    else:
      print('%s is not a valid path, please verify' % folders)
      sys.exit()
  else:
    print('Usage: python ClassVsProtonVsCom.py file name with path ') #give the file name I created with uniq chemical shift class with hmdb_id/class, no of com/class,
