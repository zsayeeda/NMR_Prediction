...
def ClassVsNoOfCmp(file_name):

  dataset_file = pd.read_csv(file_name , header = None)
  data_for_graph = pd.read_csv(file_name , sep=',')
  sns.set(font_scale=0.5)
  countplt=sns.catplot(x='chemical_shift_class', y= 'NO_OF_CMP', data=data_for_graph, kind ='bar')
  plt.title('Visualization of Chemical Shift Classes Vs No. of Compound', fontsize=10)
  plt.xlabel('Chemical Shift Class', fontsize=10)
  plt.ylabel('No. of the Compound', fontsize=10)
...
if __name__ == '__main__':
  if len(sys.argv) > 1:
    ...
    if os.path.exists(folders):
      ...
      ClassVsNoOfCmp(folders)
    else:
      print('%s is not a valid path, please verify' % folders)
      sys.exit()
  else:
    print('Usage: python ClassVsNoOfCmp.py file name with path ') #give the file name I created with uniq chemical shift class with hmdb_id/class, no of com/class,
